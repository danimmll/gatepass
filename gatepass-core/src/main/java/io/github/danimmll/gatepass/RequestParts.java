...
public record RequestParts(String method, String rawPath, String rawQuery) {
    ...
    /** The first {@code /}, {@code ?} or {@code #} at or after {@code from}. */
    private static int indexOfAny(String value, int from) {
    ...
    private static String toAscii(String value) {
        int firstNonAscii = 0;
        while (firstNonAscii < value.length() && value.charAt(firstNonAscii) <= 0x7F) {
            firstNonAscii++;
        }
        if (firstNonAscii == value.length()) {
            return value;
        }
