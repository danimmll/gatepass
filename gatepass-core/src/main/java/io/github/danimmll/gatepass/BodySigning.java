...
public record BodySigning(boolean enabled, long maxBytes) {
    ...
    /** The largest limit: bodies are held in a single array. */
    public static final long MAX_BYTES_LIMIT = Integer.MAX_VALUE - 8;

    /**
     * Validates the limit.
     *
     * @param enabled whether bodies are signed
     * @param maxBytes the largest body that is signed, in bytes, between 0 and {@link #MAX_BYTES_LIMIT}
     */
    public BodySigning {
        if (maxBytes < 0 || maxBytes > MAX_BYTES_LIMIT) {
