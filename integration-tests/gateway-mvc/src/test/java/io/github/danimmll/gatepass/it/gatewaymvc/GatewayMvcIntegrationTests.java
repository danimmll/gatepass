...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
        properties = "gatepass.secrets[0]=" + GatewayMvcIntegrationTests.SECRET)
class GatewayMvcIntegrationTests {

    static final String SECRET = "gateway-mvc-integration-secret-0123456789abcdefghij";
    ...
    void forwardsAPassBoundToThePathAndQueryStringTheServiceReceives() throws Exception {
        assertThat(send("GET", "/api/orders/5?expand=true&note=a%20b%2Bc", "forged-by-the-client")).isEqualTo(200);
    ...
    void passIsBoundToTheMethod() throws Exception {
        ...
        String pass = backend.next().passes().get(0);
        assertThat(this.service.verify(pass, RequestParts.of("DELETE", "/orders", null)).verdict())
                .isEqualTo(Verdict.INVALID);
        assertThat(this.service.verify(pass, RequestParts.of("POST", "/orders", null)).verdict())
