...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, properties = {
        "gatepass.secrets[0]=" + GatewayMvcBodySigningIntegrationTests.SECRET,
        "gatepass.body.enabled=true",
        "gatepass.body.max-size=64B" })
class GatewayMvcBodySigningIntegrationTests {

    static final String SECRET = "gateway-mvc-body-secret-0123456789abcdefghij";
    ...
    void forwardsTheBodyUntouchedWithAPassThatCoversIt() throws Exception {
        assertThat(send("POST", "/api/orders", "application/json", PIZZA)).isEqualTo(200);
    ...
    @Test
    void aFormPostIsCoveredAsTheGatewayForwardsIt() throws Exception {
        assertThat(send("POST", "/api/orders", "application/x-www-form-urlencoded", "item=pizza&size=L%26XL"))
                .isEqualTo(200);

        RecordingBackend.Received received = backend.next();
        assertThat(new String(received.body(), StandardCharsets.UTF_8)).contains("item=pizza");
        assertThat(verify(received).bodyMatches(received.body())).isTrue();
    }

    @Test
    void aRequestWithoutABodyIsCoveredToo() throws Exception {
        assertThat(send("GET", "/api/orders/1", null, null)).isEqualTo(200);
    ...
    void aBodyTooLargeToSignIsAnswered413AndNeverForwarded() throws Exception {
        assertThat(send("POST", "/api/orders", "application/json", "x".repeat(100))).isEqualTo(413);
    ...
    private int send(String method, String path, String contentType, String body) throws Exception {
